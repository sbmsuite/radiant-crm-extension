...
module Admin::TasksHelper
  ...
  def link_to_task_complete(pending, bucket)
    ...
    onclick << remote_function(:url => "/admin/tasks/complete/#{pending.id}", :method => :put, :with => "'bucket=' + \'#{bucket}\'"
)
  end
  ...
  def replace_content(task, bucket = nil)
    ...
    update_page do |page|
      page[dom_id(task)].replace_html :partial => "admin/tasks/#{partial}", :collection => [ task ], :locals => { :bucket => bucket }
    end
  end
