...
class Admin::CampaignsController < Admin::ResourceController
  layout 'crm'
  ...
  def edit
    ...
    if params[:previous] =~ /(\d+)\z/
      @previous = current_user.campaigns.find($1)
    end
  ...
  def update
    ...
    respond_to do |format|
      if @campaign.update_with_permissions(params[:campaign], params[:users])
        ...
        @users = current_site.users.except(current_user) # Need it to redraw [Edit Campaign] form.
        format.js
        format.xml  { render :xml => @campaign.errors, :status => :unprocessable_entity }
      end
    end
...
#    }
    ...
    if session[:filter_by_campaign_status]
      ...
      current_query.blank? ? Campaign.all(:conditions => records, :order => "created_at DESC").only(filtered) : Campaign.all(:conditions => records, :order => "created_at DESC").only(filtered).search(current_query)
    else
      current_query.blank? ? Campaign.all(:conditions => records, :order => "created_at DESC") : Campaign.all(:conditions => records, :order => "created_at DESC").search(current_query)
    end.paginate(pages)
  end
  ...
  def get_data_for_sidebar
    @campaign_status_total = { :all => current_user.campaigns.count, :other => 0 }
    Setting.campaign_status.keys.each do |key|
      @campaign_status_total[key] = current_user.campaigns.count(:conditions => [ "status=?", key.to_s ])
