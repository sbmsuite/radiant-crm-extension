...
class Admin::TasksController < Admin::ResourceController
  layout 'crm'
  before_filter :update_sidebar, :only => :index

  around_filter ScopedAccess::Filter.new(Task, :mine)
  ...
  #----------------------------------------------------------------------------
  def create
    # DateInputJS doesn't give us a working string so let's make it something Task can recognize
    unless params[:task][:calendar].empty?
      date = Date.parse(params[:task][:calendar])
      params[:task][:calendar] = date.to_s
    end
