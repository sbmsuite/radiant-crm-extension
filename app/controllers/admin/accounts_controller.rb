...
class Admin::AccountsController < Admin::ResourceController
  layout 'crm'
  ...
  def edit
    ...
    if params[:previous] =~ /(\d+)\z/
      @previous = current_user.accounts.find($1)
    end
  ...
  def update
    ...
    respond_to do |format|
      if @account.update_with_permissions(params[:account], params[:users])
        ...
        @users = current_site.users.except(current_user) # Need it to redraw [Edit Account] form.
        format.js
        format.xml  { render :xml => @account.errors, :status => :unprocessable_entity }
      end
    end
  ...
  def get_accounts(options = { :page => nil, :query => nil })
    ...
    if current_query.blank?
      ...
      current_user.accounts.search(current_query)
    end.paginate(pages)
  end
